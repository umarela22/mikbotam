...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	if (isset($_POST['action_save_voucher'])) {
		...
		if ($index >= 0 && isset($vouchers[$index])) {
			$vouchers[$index] = $item;
			$_SESSION['flash_msg'] = '<div class="alert alert-success mg-b-15">Berhasil memperbarui paket voucher non-saldo <strong>' . htmlspecialchars($item['Voucher']) . '</strong>!</div>';
		} else {
			$vouchers[] = $item;
			$_SESSION['flash_msg'] = '<div class="alert alert-success mg-b-15">Berhasil menambahkan paket voucher non-saldo baru <strong>' . htmlspecialchars($item['Voucher']) . '</strong>!</div>';
		}
		...
		header("Location: ./?Mikbotam=SettingsVocnonsaldo");
		exit();
		...
		if (isset($vouchers[$del_idx])) {
			...
			$json_save = json_encode($vouchers);
			upvocnon($json_save, $id);
			$_SESSION['flash_msg'] = '<div class="alert alert-info mg-b-15">Berhasil menghapus paket voucher non-saldo <strong>' . htmlspecialchars($deleted_name) . '</strong>!</div>';
		}
		header("Location: ./?Mikbotam=SettingsVocnonsaldo");
		exit();
	}
